...
## 15. HTML 編集・作成用の運用ルール
- `input/` はユーザー素材の受け取り専用。**git には含めない**（`.gitignore` 済み）。
- 編集作業は `output/` で行うこと。`input/` から必要な素材を **コピーして** から編集する。
- `input/` の内容は原本として扱い、上書き・削除をしない。
- 「新規作成」指示で `input/` が空の場合は、`output/` で **0から作成** する。
