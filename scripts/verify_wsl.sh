#!/usr/bin/env bash
# Universal lightweight verify script for mixed-language repos (WSL/Linux).
...
############################################
# Secrets scan (minimal, no network)
############################################
log "secrets scan"
"$PY_BIN" - <<'PY'
...
if [ -f "package.json" ]; then
  ...
  if [ -n "$pm" ]; then
    ...
    case "$pm" in
      ...
      yarn) yarn test || true ;; # yarn lacks --if-present; rely on script existence
    esac
    node_status=$?
    set -e
...
if command -v node >/dev/null 2>&1; then
  for test_script in scripts/test_*.js; do
    if [ ! -f "$test_script" ]; then
      continue
    fi
    log "node $test_script"
    node "$test_script"
    ran=1
  done
else
  if [ -f "scripts/test_article_search.js" ] || [ -f "scripts/test_finder_core.js" ]; then
    log "skip node scripts (node not installed)"
  fi
fi
...
############################################
# Dotnet (opt-in to avoid OOM on Unity/WSL)
############################################
