...
const getPrimaryWorkImage = (work) =>
  ...
  work?.hoverImageUrl ||
  ensureArray(work?.galleryImages).map((item) => item?.url).find(Boolean) ||
  ensureArray(work?.galleryImageUrls).find(Boolean) ||
  DEFAULT_OG_IMAGE;
...
const getListingWorkImage = (work) =>
  getAllWorkImages(work)[0] || DEFAULT_OG_IMAGE;
    ...
    <article class="card work-card work-card--compact" data-tone="${getVisualTone(work.id || work.slug || work.title)}">
      ...
      <div class="work-card__body">
        ...
        <div class="work-card__meta">
          ...
          <span class="help">${escapeHtml(work.highlightPoints?.[0] || "条件要約あり")}</span>
        </div>
      ...
      <p class="help work-card__footnote">${escapeHtml(
        [work.format || "作品", releaseText].filter(Boolean).join(" / ")
      )}</p>
    </article>
  `;
};
...
const renderWorkGallery = (work) => {
  ...
  return `
    <section class="detail-work-card detail-work-cover detail-work-cover--article">
      ...
      <p class="pdp-gallery__caption">${mediaUrls.length}枚のメディアを確認できます。</p>
    </section>
  `;
};
...
const renderHomeSpotlightBanner = ({ profile, collection, work, relatedWorks = [] }) => `
  <a class="home-showcase-banner home-showcase-banner--spotlight" href="${escapeHtml(
    ...
    <div class="home-showcase-banner__copy">
      ...
      <p class="home-showcase-banner__description">入口作品から条件検索、固定特集、記事ガイドまでをひと続きで辿れる導線をトップにまとめています。</p>
  ...
  <article class="home-showcase-product">
    ...
    <div class="home-showcase-product__body">
      ...
      <div class="home-showcase-product__footer">
        <strong class="home-showcase-product__price">${escapeHtml(
          work.releasedAt ? `公開 ${work.releasedAt}` : "公開日未設定"
        )}</strong>
        <span class="home-showcase-product__detail">${escapeHtml(
          `${Math.max(1, getAllWorkImages(work).length || 0)}枚`
        )}</span>
      </div>
    </div>
  </article>
...
const renderHomePage = () => {
  ...
  const homePageDescription =
    "ケモホモ作品を、オスケモ寄り、ケモ率、TF、媒体、除外条件などの細かい条件から探せる作品ファインダーです。";
  ...
  const collageWorks = publishedWorks.slice(0, 6);
  ...
  const categoryItems = [
    ...
    { label: "除外条件", meta: "苦手条件を先に外す", href: "/builder/", icon: "tag" },
  ];
  const websiteLd = {
...
const renderArticlesIndexPage = () => {
  ...
  const listLd = {
    ...
    description: "比較記事、使い方ガイド、レビュー設計メモをまとめた記事一覧です。",
    mainEntity: {
      "@type": "ItemList",
          ...
          <header class="detail-header">
            ...
            <div class="detail-fact-grid">
              <div class="detail-fact">
                <span>更新</span>
                <strong>${escapeHtml(work.updatedAt || work.releasedAt || "未設定")}</strong>
              </div>
              <div class="detail-fact">
                <span>外部導線</span>
                <strong>${publicExternalLinks.length}件</strong>
              </div>
              <div class="detail-fact">
                <span>関連特集</span>
                <strong>${ensureArray(work.collectionIds).length}件</strong>
              </div>
            </div>
          </header>
